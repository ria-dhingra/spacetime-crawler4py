...
def tokenize_string(text: str) -> list[str]:
   return [w for w in re.findall(r'[a-zA-Z]+', text.lower()) if len(w) <= 30]
...
def scraper(url, resp):
   ...
   if resp.status == 200 and resp.raw_response and clean_url not in unique_urls:
       ...
       if len(tokens) > 25000:
           print(f"[TRAP] Skipping outlier page with {len(tokens)} tokens: {clean_url}")
...
def is_valid(url):
   ...
   try:
       ...
       return not re.match(
           ...
           r"|rm|smil|wmv|swf|wma|zip|rar|gz|mpg|ppsx|pps|ps)$", parsed.path.lower())
